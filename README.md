...
The panel sits in the bottom-right corner, only as tall as its controls. A chevron hides it, and a small button in that corner brings it back. [DialKit](https://www.dialkit.dev/) holds the text, the typeface menu (LT Superior Semibold, then Inter Semibold), letter spacing, the point-count dial, a grid toggle, export SVG, and reset. Grid size and snap appear only while the grid is on. Snap pulls the nearer edge of a drag onto a grid line. The grid covers the whole screen. A mouse wheel or pinch zooms. A trackpad swipe, middle-mouse drag, or the arrow keys move the canvas. Press **Fit to screen** to place the word in the middle of the window. The point dial starts sparse: 1 is the fewest anchors, 8 (the middle) is every anchor on the typeface, and 15 adds points along the curves. Export SVG downloads the outlines cropped to the letters. The point dial only changes how many anchors appear on hover. A sun/moon icon in the panel’s top-right switches light and dark. The title is Inter Medium at 16px. Changing the text, typeface, or letter spacing clears any drags.
